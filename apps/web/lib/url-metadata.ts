...
export function getDomainFromUrl(url: string, hostNameOnly = false): string {
  try {
    if (hostNameOnly) return new URL(url).hostname.replace("www.", "");
    return new URL(url).href.replace("www.", "");
  } catch {
    return url;
  }
}
